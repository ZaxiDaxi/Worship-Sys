// src/components/SongCreate/SongCreate.tsx
...
const SongCreate: React.FC = () => {
  ...
  // Initialize with a default chord input so the field is shown on load
  const [lines, setLines] = useState<LyricLine[]>([{ text: "", chords: [{ chord: "", position: 0 }] }]);
  ...
  const addLine = () => {
    updateLines([...lines, { text: "", chords: [{ chord: "", position: 0 }] }]);
  };
  ...
  return (
    <div className="relative flex min-h-screen bg-white">
      {/* Pass a callback or prop to Sidebar so that it can update sidebarOpen */}
                  ...
                  <div key={i}>
                    <ChordLine
                      ...
                      onChange={(newText, newChords) =>
                        handleLineChange(i, { text: newText, chords: newChords })
                      }
                    />
                    {lines.length > 1 && (
                      <Button
      ...
      {/* Floating Edit Toolbar now hides when sidebarOpen is true */}
      <EditToolbar onUndo={undo} onRedo={redo} onAdd={addLine} isSidebarOpen={sidebarOpen} />
